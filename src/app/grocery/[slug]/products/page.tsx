...
const EMPTY_FORM = { name: '', price: '', mrp: '', weight: '' }

export default function GrocerySlugProductsPage() {
  ...
  const loadProducts = useCallback(async (partnerId: string) => {
    const { data } = await supabase
      ...
      .select('id, name, image_url, price, mrp, weight, in_stock, is_active')
  ...
  function openEdit(p: Product) {
    setForm({ name: p.name, price: String(p.price), mrp: p.mrp !== null ? String(p.mrp) : '', weight: p.weight ?? '' })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (editItem) {
      const { error } = await supabase
        .from('grocery_products')
        .update({ name: form.name, price, mrp, weight: form.weight || null, image_url: formImage })
        .eq('id', editItem.id)
      if (error) { setFormError(error.message); setSaving(false); return }
      setProducts((prev) => prev.map((p) => p.id === editItem.id
        ? { ...p, name: form.name, price, mrp, weight: form.weight || null, image_url: formImage }
        : p))
    } else {
