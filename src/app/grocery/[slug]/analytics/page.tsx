...
export default function GrocerySlugAnalyticsPage() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true)
      const { data: part } = await supabase.from('grocery_partners').select('id, name, slug').eq('slug', slug).single()
      ...
      <main className="flex-1 overflow-auto">
        <div className="bg-white border-b border-[#E5E7EB] px-6 py-5 sticky top-0 z-10 space-y-3">
          <div>
            ...
            <p className="text-[12.5px] text-[#9CA3AF]">Your grocery partner performance</p>
          </div>
