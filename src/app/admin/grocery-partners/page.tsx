...
import { Plus, Search, Trash2, ToggleLeft, ToggleRight, Loader2, CheckCircle, AlertCircle, Store, Copy } from 'lucide-react'
...
export default function AdminGroceryPartnersPage() {
  ...
  const load = useCallback(async () => {
    const { data: parts } = await supabase.from('grocery_partners')
      .select('id, name, slug, phone, address, is_active')
      .order('created_at', { ascending: false })
    setPartners((parts as Partner[]) ?? [])

    const { data: ords } = await supabase.from('orders').select('grocery_partner_id').not('grocery_partner_id', 'is', null)
    const counts: Record<string, number> = {}
    for (const o of ords ?? []) {
      const id = (o as { grocery_partner_id: string }).grocery_partner_id
  ...
  async function addPartner(e: React.FormEvent) {
    ...
    const { error: err } = await supabase.from('grocery_partners').insert({
      name: form.name,
      slug,
  ...
  async function toggleActive(p: Partner) {
    await supabase.from('grocery_partners').update({ is_active: !p.is_active }).eq('id', p.id)
    setPartners((prev) => prev.map((x) => x.id === p.id ? { ...x, is_active: !p.is_active } : x))
  }

  async function deletePartner(id: string) {
    if (!confirm('Remove this grocery partner? Any orders assigned to them will become unassigned.')) return
    await supabase.from('grocery_partners').delete().eq('id', id)
    setPartners((prev) => prev.filter((x) => x.id !== id))
  }

  function copyLink(p: Partner) {
    const url = `${window.location.origin}/grocery/${p.slug}/orders`
      ...
      <main className="flex-1 overflow-auto">
        <div className="bg-white border-b border-[#E5E7EB] px-6 py-5 sticky top-0 z-10">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h1 className="text-[20px] font-[800] text-[#111827]" style={{ fontWeight: 800 }}>Grocery Partners</h1>
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => setShowModal(true)}
                ...
                <Plus className="w-4 h-4" /> Add Grocery Partner
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="flex flex-col items-center gap-3 py-20 text-[#9CA3AF]">
              <Store className="w-12 h-12" strokeWidth={1} />
              <p className="text-[15px] font-semibold text-[#374151]">No grocery partners yet</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6 z-10">
            <h2 className="text-[17px] font-[800] text-[#111827] mb-5 flex items-center gap-2">
              <Store className="w-5 h-5 text-[#7C3AED]" /> Add Grocery Partner
            </h2>
            {error && (
            ...
            <form className="space-y-4" onSubmit={addPartner}>
              <div>
                ...
                <input value={form.name} onChange={(e) => setF('name', e.target.value)} placeholder="e.g. Sharma Kirana Store" required
