...
import { Plus, Search, Trash2, ToggleLeft, ToggleRight, Loader2, CheckCircle, AlertCircle, ShoppingBasket, Copy } from 'lucide-react'
...
export default function AdminStorePartnersPage() {
  ...
  const load = useCallback(async () => {
    const { data: parts } = await supabase.from('store_partners')
      .select('id, name, slug, phone, address, is_active')
      .order('created_at', { ascending: false })
    setPartners((parts as Partner[]) ?? [])

    const { data: ords } = await supabase.from('orders').select('store_partner_id').not('store_partner_id', 'is', null)
    const counts: Record<string, number> = {}
    for (const o of ords ?? []) {
      const id = (o as { store_partner_id: string }).store_partner_id
  ...
  async function addPartner(e: React.FormEvent) {
    ...
    const { error: err } = await supabase.from('store_partners').insert({
      name: form.name,
      slug,
  ...
  async function toggleActive(p: Partner) {
    await supabase.from('store_partners').update({ is_active: !p.is_active }).eq('id', p.id)
    setPartners((prev) => prev.map((x) => x.id === p.id ? { ...x, is_active: !p.is_active } : x))
  }

  async function deletePartner(id: string) {
    if (!confirm('Remove this store partner? Any orders assigned to them will become unassigned.')) return
    await supabase.from('store_partners').delete().eq('id', id)
    setPartners((prev) => prev.filter((x) => x.id !== id))
  }

  function copyLink(p: Partner) {
    const url = `${window.location.origin}/store/${p.slug}/orders`
      ...
      <main className="flex-1 overflow-auto">
        <div className="bg-white border-b border-[#E5E7EB] px-6 py-5 sticky top-0 z-10">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h1 className="text-[20px] font-[800] text-[#111827]" style={{ fontWeight: 800 }}>Store Partners</h1>
              <p className="text-[12.5px] text-[#9CA3AF]">{partners.length} partners · {partners.filter((p) => p.is_active).length} active</p>
            </div>
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => setShowModal(true)}
                ...
                <Plus className="w-4 h-4" /> Add Store Partner
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="flex flex-col items-center gap-3 py-20 text-[#9CA3AF]">
              <ShoppingBasket className="w-12 h-12" strokeWidth={1} />
              <p className="text-[15px] font-semibold text-[#374151]">No store partners yet</p>
              <button onClick={() => setShowModal(true)} className="text-[13px] text-[#7C3AED] font-medium">+ Add your first partner</button>
            </div>
          ) : (
            <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
              {filtered.map((p) => (
                <div key={p.id} className={cn('bg-white rounded-2xl border border-[#E5E7EB] p-4 shadow-zippy-sm', !p.is_active && 'opacity-60')}>
                  <div className="flex items-start gap-3 mb-3">
                    <div className="w-11 h-11 rounded-xl bg-[#F5F3FF] flex items-center justify-center shrink-0">
                      <ShoppingBasket className="w-5 h-5 text-[#7C3AED]" />
                    </div>
                  ...
                  {p.address && (
                    <p className="text-[11.5px] text-[#6B7280] mb-3 line-clamp-1">{p.address}</p>
                  )}
                  ...
                  <div className="grid grid-cols-1 gap-2 mb-3">
                    <div className="bg-[#F8FAFC] rounded-lg p-2 text-center">
                      ...
                      <div className="text-[10px] text-[#9CA3AF]">Assigned Orders</div>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6 z-10">
            <h2 className="text-[17px] font-[800] text-[#111827] mb-5 flex items-center gap-2">
              <ShoppingBasket className="w-5 h-5 text-[#7C3AED]" /> Add Store Partner
            </h2>
            {error && (
            ...
            <form className="space-y-4" onSubmit={addPartner}>
              <div>
                <label className="block text-[12.5px] font-[600] text-[#374151] mb-1.5">Shop Name *</label>
                <input value={form.name} onChange={(e) => setF('name', e.target.value)} placeholder="e.g. City Grocery Mart" required
              ...
              <div>
                <label className="block text-[12.5px] font-[600] text-[#374151] mb-1.5">Address</label>
                <input value={form.address} onChange={(e) => setF('address', e.target.value)} placeholder="Shop address"
                  className="w-full px-3.5 py-2.5 border border-[#E5E7EB] rounded-xl text-[13.5px] outline-none focus:border-[#7C3AED] transition-all" />
              </div>
